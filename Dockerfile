...
RUN apt-get build-dep -y linux-image-$(uname -r) >/dev/null && \
    apt-get install -y fakeroot cpio bc libssl-dev gawk wget kmod dkms bison flex libelf-dev fakeroot build-essential crash kexec-tools makedumpfile kernel-wedge libncurses5 libncurses5-dev libelf-dev asciidoc binutils-dev libudev-dev pciutils-dev rsync libiberty-dev linux-libc-dev && \
